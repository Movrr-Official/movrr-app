...
export default async function DashboardCampaignDetailPage({
  ...
  if (!session || session.appUser.role !== "advertiser")
  ...
  return (
    <div className="min-h-screen gradient-bg px-4 sm:px-6 py-8 md:py-12 lg:py-16 lg:pt-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <Badge
          variant={
            campaign.lifecycleStatus === "active" ? "success" : "secondary"
          }
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4">
        <StatsCard
          ...
          description="Configured campaign budget"
          icon={CircleDollarSign}
        />
        <StatsCard
          title="Impressions"
          value={String(campaign.impressions)}
          description="Recorded impression volume"
          icon={LineChart}
        />
        <StatsCard
          title="QR scans"
          value={String(campaign.qrScans)}
          description="Attributed QR scan volume"
          icon={ScanLine}
        />
        <StatsCard
          title="Riders"
          value={String(campaign.riders.length)}
          description="Visible participant count"
          icon={Users}
        />
      </div>
        ...
        <div className="space-y-6">
          <Card className="glass-card border-border/60">
            <CardHeader>
              ...
              <CardDescription>
                Read-only campaign configuration and pacing posture published by
                MOVRR operations.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-xl border border-border/60 bg-background/70 p-4">
                <p className="text-sm text-muted-foreground">Target zones</p>
                <p className="mt-2 font-medium">
                  {campaign.targetZones.length || campaign.metadata.zoneCount}
                </p>
              </div>
              <div className="rounded-xl border border-border/60 bg-background/70 p-4">
                <p className="text-sm text-muted-foreground">Date window</p>
                <p className="mt-2 font-medium">
                  {formatRelativeDate(campaign.startDate)} to {formatRelativeDate(campaign.endDate)}
                </p>
              </div>
              <div className="rounded-xl border border-border/60 bg-background/70 p-4">
                <p className="text-sm text-muted-foreground">Pacing health</p>
                <p className="mt-2 font-medium capitalize">
                  {campaign.metadata.pacingHealth}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-border/60">
            <CardHeader>
              ...
              <CardDescription>
                Campaign delivery geography adapted from the admin zone model.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-border/60 bg-background/70 p-4">
                ...
                <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
                  {(campaign.hotZones.length
                    ...
                    (zone: {
                      ...
                      <li key={zone.id}>
                        {zone.name || "Unnamed hot zone"}
                        {zone.bonus_percent ? ` +${zone.bonus_percent}%` : ""}
                      </li>
                    ),
                  )}
                </ul>
        ...
        <div className="space-y-6">
          <Card className="glass-card border-border/60">
            <CardHeader>
              ...
              <CardDescription>
                Status progression and freshness cues for this campaign.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {campaign.timeline.length ? (
                ...
                <p className="text-sm text-muted-foreground">
                  No campaign timeline events are visible yet.
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-border/60">
            <CardHeader>
              ...
              <CardDescription>
                Participant identities visible to the authenticated advertiser
                account only.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {campaign.riders.length ? (
                  ...
                  (rider: {
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground">
                        {[rider.city, rider.country].filter(Boolean).join(", ") || "Location unavailable"}
                      </p>
                    </div>
                  ),
                )
                ...
                <p className="text-sm text-muted-foreground">
                  No rider participation is visible for this campaign yet.
                </p>
              )}
            </CardContent>
          </Card>
